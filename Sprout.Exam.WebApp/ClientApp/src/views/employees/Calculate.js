...
export class EmployeeCalculate extends Component {
  ...
  handleSubmit(e){
      ...
      if (this.state.typeId == 1) {
          if (isNaN(parseFloat(this.state.absentDays))) {
              alert('Please enter the number of absent days.');
              return;
          }
      }
      if (this.state.typeId == 2) {
          if (isNaN(parseFloat(this.state.workedDays))) {
              alert('Please enter the number of worked days.');
              return;
          }
      }
...
<button type="button" onClick={() => this.props.history.push("/employees/index")} className="btn btn-primary">Back</button>
  ...
  async calculateSalary() {
    ...
    const requestOptions = {
        ...
        body: JSON.stringify({employeeid: this.state.id,absentDays: this.state.absentDays,workedDays: this.state.workedDays})
    };
    const response = await fetch('api/employees/' + this.state.id + '/calculate',requestOptions);
    const data = await response.json();
      this.setState({
          loadingCalculate: false, netIncome: data.toLocaleString(undefined, {
